...
<section class="">
	<div class="absolute w-full h-[calc(100vh-4.25rem)] object-cover overflow-hidden">
		<h1 class="text-[12rem] w-full h-full text-base-content/80">
			C O N V E N I E N C E E F F I C I E N C Y B R A N D I N G A N A L Y T I C S S E C U R I T Y
		</h1>
	</div>
	<div class="absolute w-full h-[calc(100vh-4.25rem)] overflow-hidden">
		<img src={box_img} alt="" class="object-cover w-full h-full" />
	</div>
	<div
...
<style lang="scss">
	.login-div {
		...
		&::after {
			...
			left: -2px;
			right: -2px;
			background: linear-gradient(180deg, hsl(var(--p)), transparent, transparent, transparent);
			content: '';
			z-index: -50;
			border-radius: 1rem;
		}
